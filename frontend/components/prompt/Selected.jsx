...
function Option({ children, className, optionId }) {
 ...
 return (
  <div
   onClick={handleClick}
   className={cn(
    'cursor-pointer border-2 border-accent p-2 sm:p-4 text-center text-accent font-semibold bg-bg-secondary rounded-md ',
    className,
    isSmall && 'w-auto mx-4'
   )}>
   {children}
 ...
 return (
  <div className={cn('flex gap-4 w-full justify-center mt-3 mb-3', isSmall && 'flex-col')}>
   ...
   <div
    ...
    className={cn(
     'grid place-content-center border-2 border-accent rounded-md cursor-pointer font-semibold w-14 text-4xl text-accent bg-bg-secondary',
     isSmall && 'w-24 m-auto'
    )}>
    ?
   </div>
